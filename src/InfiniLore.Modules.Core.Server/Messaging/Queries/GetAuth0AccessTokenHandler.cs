...
public class GetAuth0AccessTokenHandler(
    ...
    protected override Server.Result<IAuth0AccessToken> AccessDeniedResult => throw new NotImplementedException();
    
    // -----------------------------------------------------------------------------------------------------------------
    // Constructors
    // -----------------------------------------------------------------------------------------------------------------
    protected override async Task<Server.Result<IAuth0AccessToken>> HandleCommandAsync(GetAuth0AccessTokenQuery command, CancellationToken ct = default) {
        ...
        if (storeResult.IsError) {
            ...
            return Server.Result<IAuth0AccessToken>.FromError("Cannot get auth0 access token. Key not found.");
        }
        ...
        if (store.Value.IsNullOrEmpty()) {
            ...
            return Server.Result<IAuth0AccessToken>.FromError("Cannot get auth0 access token. Value is empty.");
        }
        ...
        if (!store.TryGetConvertJsonValueToObject(out Auth0AccessTokenJsonDto? dto)) {
            ...
            return Server.Result<IAuth0AccessToken>.FromError("Cannot get auth0 access token. Json conversion failed.");
        }
