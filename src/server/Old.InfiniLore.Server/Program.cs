...
using Old.InfiniLore.Database.Seeding;
using Old.InfiniLore.Database.Seeding.Content.Account;
using Old.InfiniLore.Database.Seeding.Content.Data.System;
using Old.InfiniLore.Server.API;
using Old.InfiniLore.Server.Components;
using Old.InfiniLore.Server.Services;
using Old.InfiniLore.Server.Services.Authentication;
using Old.InfiniLore.Server.Services.Authorization;
using Old.InfiniLore.Server.Services.CQRS.PipelineBehaviours;
...
namespace Old.InfiniLore.Server;
// ---------------------------------------------------------------------------------------------------------------------
// Code
// ---------------------------------------------------------------------------------------------------------------------
public static class Program {
    public static async Task Main(string[] args) {
        ...
        builder.Services.RegisterServicesFromOldInfiniLoreDatabase();// Registers the IUnitOfWorkDb<T>
        #endregion
        ...
        builder.Services.AddMediatR(cfg => {
            cfg.RegisterServicesFromAssemblyContaining<Old.InfiniLore.Server.Services.CQRS.Handlers.ICqrsHandlersAssemblyEntry>();
        ...
        builder.Services.RegisterServicesFromOldInfiniLoreDatabaseSeeding();
        ...
        builder.Services.RegisterServicesFromOldInfiniLoreServerServicesAuthorization();
        builder.Services.RegisterServicesFromOldInfiniLoreServerServicesAuthentication();
        builder.Services.RegisterServicesFromOldInfiniLoreServerServices();
        ...
        app.MapRazorComponents<App>()
            ...
            .AddAdditionalAssemblies(typeof(Old.InfiniLore.WasmClient.IAssemblyEntry).Assembly);

        app.UseDefaultExceptionHandler()
            .UseFastEndpoints(ctx => {
                ctx.Endpoints.RoutePrefix = "api/v1";
                ctx.Binding.ReflectionCache.AddFromOldInfiniLoreServerAPI();
