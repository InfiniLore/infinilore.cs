...
public interface IInfiniLoreUserRepository : IBasicDataRepository<IInfiniLoreUser> {
    ValueTask<Result<IInfiniLoreUserAuth0Data[]>> TryGetAllByAuth0IdsAsync(QueryConfig config = default, CancellationToken ct = default, params HashSet<string> authIds);

    ValueTask<Result<IInfiniLoreUserAuth0Data>> TryGetByAuth0IdAsync(string auth0Id, QueryConfig config = default, CancellationToken ct = default);
