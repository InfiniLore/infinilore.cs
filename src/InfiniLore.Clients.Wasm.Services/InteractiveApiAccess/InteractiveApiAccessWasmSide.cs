...
public class InteractiveApiAccessWasmSide(
    ...
    public async ValueTask<Result> GetLoreScopesAsync(string userId, CancellationToken ct = default) {
        ...
        try {
            ...
            if (result is null) return Result.FromError("Could not get data from API");
            ...
            // if (response is null) return Result.FromError("Could not deserialize data from API");

            return Result.FromState(true);
        }
        catch (Exception e) {
            logger.Error(e, "Failed to get LoreScopes for user {userId} because '{reason}'", userId, e.Message);
            return Result.FromError($"Unknown failure");
        }
    }
    #endregion
