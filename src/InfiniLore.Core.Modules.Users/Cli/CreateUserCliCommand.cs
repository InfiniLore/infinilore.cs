...
using FastEndpoints;
using InfiniLore.Core.Modules.Users.Messaging;
...
public partial class CreateUserCliCommand(ILogger<CreateUserCliCommand> logger) : ICliCommand<CreateUserCliParameters>{
    ...
    // Methods
    // -----------------------------------------------------------------------------------------------------------------
    public async ValueTask ExecuteAsync(CreateUserCliParameters parameters, CancellationToken ct = new CancellationToken()) {
