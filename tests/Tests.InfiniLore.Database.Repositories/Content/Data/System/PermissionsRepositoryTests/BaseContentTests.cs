...
public class BaseContentTests(DatabaseInfrastructure infrastructure) : BaseContentRepositoryTestFramework<PermissionsRepository, InfinilorePermission>(infrastructure) {
    ...
    public Faker<InfinilorePermission> PermissionFaker = new Faker<InfinilorePermission>()
        .RuleFor(p => p.Id, _ => Guid.NewGuid()) // Just use a random guid for the id, instead of the version7 guid.
        .RuleFor(p => p.Name, _ => Truncate(Guid.NewGuid().ToString(), MaxNameLength)) // To ensure they are unique
        .RuleFor(p => p.Description, f => Truncate(f.Lorem.Sentence(), MaxDescriptionLength));
